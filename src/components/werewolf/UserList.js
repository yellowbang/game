...
import {setIsKilled, deleteUser} from "../../store/actions/werewolfActions";
...
class UserList extends Component {
    ...
    setIsKilled = (killedUser) => {
        this.props.setIsKilled(killedUser);
    };

    render() {
        let {werewolfUsers, isMc = false} = this.props;

        werewolfUsers = _.sortBy(werewolfUsers, [(user) => {
            return user.number;
        }]);
            ...
            <div className="container" style={{marginTop: 10}}>
                {
                    _.map(werewolfUsers, (user) => {
                        let userContainer = {...userContainerStyle, opacity: user.death ? 0.3 : 1};
                        return (
                            <div id={user.id} key={user.username} style={userContainer}>
                                ...
                                {
                                    isMc && <div style={{...itemDetailStyle, flex: 3}}>{user.role}</div>
                                }
                                {
                                    isMc &&
                                    <button
                                        ...
                                        onClick={this.setIsKilled.bind(this, user)}>
                                        Killed
                                    </button>
                                }
                            </div>
